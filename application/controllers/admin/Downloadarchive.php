...
class Downloadarchive extends CI_Controller {
    ...
    public function index() {
		...
		if(!empty($this->input->get('deleteAllSearch')) && $this->input->get('deleteAllSearch')=='true' && $this->session->userdata('role')=='ADMIN'){
			...
			redirect(site_url('admin/downloadarchive?website_id='.$condition['res.website_id']),'refresh');
		}
		...
		$config['per_page'] = 500;
		$config['total_rows'] = $this->archive_model->get_job_listings_total($condition,$botIdArr);
		...
		$jobListings = $this->archive_model->get_job_listings($condition, $config['per_page'], $page,$sort_order,$botIdArr);
		$data['jobListings'] = $jobListings;
		$data['total_rows'] = $config['total_rows'];
		$data['main_content'] =$this->load->view('admin/download-archive/searchform', $data, TRUE);
		if($data['website_id']==1){
			$data['main_content'] .= $this->load->view('admin/download-archive/craiglist', $data, TRUE);
		}elseif($data['website_id']==2){
			$data['main_content'] .= $this->load->view('admin/download-archive/linkedin', $data, TRUE);
		}elseif($data['website_id']==3){
			$data['main_content'] .= $this->load->view('admin/download-archive/indeed', $data, TRUE);
		}
		$data['main_content'] .= $this->load->view('admin/download-archive/download_footer', $data, TRUE);
	...
	function fetchDetails($result_id){
		...
		$jobDetails=$this->archive_model->get_job_details($condition);
	...
	function downloadCSV(){
		...
		$data['website_id']=$defaultWebsiteId;
		$data['websiteListArr'] = $websiteListArr;
		$jobListings = $this->archive_model->get_job_listings($condition,'','','',$botIdArr);
		...
		foreach($jobListings as $jobListing){
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $categoriesArr[$categoryId]); 			$objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, $subcategoriesArr[$jobListing['sub_category_id']]); 
	...
	public function delete() {
		...
		if(!empty($record_delete)){
			$this->archive_model->deleteByIn('results','result_id',$record_delete);
		...
		redirect(site_url('admin/downloadarchive?'.$url),'refresh');
	}	
}
