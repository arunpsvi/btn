...
class Schedule extends CI_Controller {
    ...
    public function index() {
		...
		if(in_array($this->session->userdata('role'), $this->config->item('salesAccess'))){
			...
			$searches=$this->common_model->get_all_searches($condition);
		}
	...
	public function addSchedule($keywordId) {
		
		$keywordData = $this->common_model->getSearchByid($keywordId);
		$condition=Array();
		$condition['keyword_id']=$keywordId;
		...
		if ($this->form_validation->run() == false) {
			$schedules=$this->common_model->get_all_schedules($condition);

			$data['searches'] = $searches;
			$data['keywordData'] = $keywordData;
			$data['websiteListNameArr'] = $websiteListNameArr;
			$data['schedules'] = $schedules;
			$data['main_content'] = $this->load->view('admin/schedule/addschedule', $data, TRUE);
			$this->load->view('admin/index', $data);
		}else{
			...
			for($i=1;$i<=$numdays;$i++){
				...
				if($dataArray[$dayOfWeek]=='Y'){
					...
					$this->common_model->insert($data, 'schedules');
				}

				$next_date = date('Y-m-d', strtotime($next_date .' +1 day'));				
			}
			redirect(site_url('admin/schedule/addSchedule/'.$keywordId),'refresh');
		}
	}
	...
	public function view($scheduleId) {
		...
		$condition=Array();
		$condition['sr_id']=$scheduleId;
		#$condition['scheduled_time']=date('Y-m-d h:i:sa');

		$schedules=$this->common_model->get_upcoming_schedules($condition);
		$data['schedules'] = $schedules;
		$data['keywordData'] = $keywordData;
		$data['websiteListNameArr'] = $websiteListNameArr;
		$data['schedules'] = $schedules;
		$data['main_content'] = $this->load->view('admin/schedule/list', $data, TRUE);
		$this->load->view('admin/index', $data);
	}


	function deleteSchedule($searchId,$keywordId){		
		$condition = array(
			'id' => $searchId
		);
		$this->common_model->delete('schedule_records',$condition);
		$condition = array(
			'sr_id' => $searchId
		);
		$this->common_model->delete('schedules',$condition);

		$this->session->set_flashdata('msg', 'Schedule deleted successfully');
		print site_url('admin/schedule/addSchedule/'.$keywordId);
		#redirect(site_url('admin/schedule/addSchedule/'.$keywordId),'refresh');
	}
}
