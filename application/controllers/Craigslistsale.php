...
class Craigslistsale extends CI_Controller {
    ...
    public function index($cronSID='',$scheduleID='') {
		...
		if(!empty($sid)){
			...
			#$this->writeToFile('testing.txt',"cron here ".$sid);
			$manualScheduleId=0;
		    if(!empty($scheduleID)){
		        $dataToUpdate=Array();
			...
			*/
			...
			if($searchData[0]['website_id']==4){
				$condition['website_id']=1; # Only for sale as craiglist sale do also use same suburls as website id 1
			}
			...
			$this->scrape_all = $this->db->get_where('keywords',array('keyword_id'=>$sid))->row()->scrape_all;
			...
			foreach($websiteLists as $websiteList){
				...
				#$this->appendFile('log.txt',"going for ".$websiteList['sub_url']."\n");
				#$websiteList['sub_url']='https://akroncanton.craigslist.org';
				foreach($categoryLists as $categoryList){
					
					$pagination=true;
					$url=$websiteList['sub_url'].$categoryList['sub_cat_url'].'?query='.urlencode($keywords);
					if($this->scrape_all=='Y'){
						$url=$websiteList['sub_url'].$categoryList['sub_cat_url'].'?sort=date';
					}
					
					while($pagination){
						$resultFile=$this->fetchPage($url);
						#$this->appendFile('log.txt',"\tgoing for ".$url."\n");
						#$this->writeToFile('arun.html',$resultFile);
						if(preg_match('/<link\s*rel="next"\s*href="(.*?)">/is',$resultFile,$matcher)){
							$url=$matcher[1];
	...
	function fetchListings($resultFile,$categoryList,$keywords,$websiteList,$posted_date_start,$posted_date_end,$sid){
		...
		while(preg_match('/<li\s*class="result-row"\s*data-pid="\d+".*?>\s*<a\s*href="(.*?)"/is',$resultFile,$matcher)){
			...
			#print "$postDateWebsite,$posted_date_start,$posted_date_end\n";
			#Skip post if older than 60 days
			$records_to_keep=$this->db->get_where('websites',array('id'=>'4'))->row()->records_to_keep;
			$dateDiff=svi_datediff($postDateWebsite,date('Y-m-d'));
			#print "$counter --> $postDateWebsite -->$dateDiff \n";
			if($dateDiff>$records_to_keep){
				continue;
			}
			
			$resultId = $this->db->get_where('saleresults',array('job_url'=>$url))->row()->result_id;
			if(empty($resultId)){
	...
	function scrapeData($url,$categoryList,$keywords,$websiteList,$sid){
		...
		$tempFile=$resultFile;
		$urlCounter=1;
		while(preg_match('/"shortid":".*?","url":"(.*?)"/is',$tempFile,$matcher2)){
			$key="image".$urlCounter;
			$resultData[$key]=$matcher2[1];
			$tempFile=$this->after($matcher2[0],$resultFile);
			$urlCounter++;
			if($urlCounter>3){
				break;
			}
		...
		if(preg_match('/<span id="titletextonly">(.*?)<\/span>/is',$resultFile,$matcher)){
			$resultData['title']=html_entity_decode($matcher[1]);
		}
		if(preg_match('/<span class="price">(.*?)<\/span>/is',$resultFile,$matcher)){
			$resultData['price']=$matcher[1];
		}
		...
		if(preg_match('/>post id:\s*(\d+)<\/p>/is',$resultFile,$matcher)){
			$resultData['unique_id']=$matcher[1];
		}
		if(preg_match('/<div\s*class="mapaddress">(.*?)<\/div>/is',$resultFile,$matcher)){
			$resultData['address']=$matcher[1];
		}
		if(preg_match('/data-latitude="(.*?)"\s*data-longitude="(.*?)"/is',$resultFile,$matcher)){
			$resultData['latitude']=$matcher[1];
			$resultData['longitude']=$matcher[2];
		}
		$attrgroup=Array();
		if(preg_match('/<p class="attrgroup".*?>(.*?)<\/p>/is',$resultFile,$matcher)){
			$attrgroupHtml=$matcher[1];
			$attributeCounter=0;
			while(preg_match('/<span>(.*?)\s*:\s+<b>(.*?)<\/b>\s*<\/span>/is',$attrgroupHtml,$matcher3)){
				$attrgroupHtml=$this->after($matcher3[0],$attrgroupHtml);
				$key=$matcher3[1];
				$value=$matcher3[2];
				$attrgroup[$key]=$value;
				$attributeCounter++;
			...
			if($attributeCounter>0){
				$resultData['attributes']=json_encode($attrgroup);
				#print_r($resultData['attribute']);exit;
			}
		}
		...
		if($this->checkKeywords($resultFile,$keywords)){
			...
			$this->common_model->insert($resultData, 'saleresults');
		}
	}
	...
	function fetchPageNOTInUSE($url){
	    
	    $htmlPage='';
	    #Retry for 3 times in case proxy is not working...
	    $i=0;
	    while($i<3){
            $htmlPage=$this->getHtmlPage($url);
            if($htmlPage !=''){
              break;  
            }
            $i++;
