...
$(document).ready(function(){
	...
	$('#website_id').on('change', function() {
		var url='<?php echo site_url("admin/salerecordsarchive"); ?>';
		url=url+"?website_id="+$('#website_id').val();		
		location.href=url;
	});
	$('#download').on('click', function() {
		var url='<?php echo site_url("admin/salerecordsarchive/downloadCSV"); ?>';
		url=url+"?website_id="+$('#website_id').val();
	...
	$('.viewJob').on('click', function() {
		...
		var jid=jobIdArr[1];
		var apiUrl='<?php echo site_url('admin/salerecordsarchive/fetchDetails/') ?>';
		apiUrl=apiUrl+jid;
		$.ajax({
			url:apiUrl,
			method: 'get',				
